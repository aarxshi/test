...
const GPS_MIN_ACCURACY_M   = 35;   // fixes worse than this are ignored outright
...
function onGPSFix(pos) {
  ...
  // 1. Reject fixes that are too imprecise to be useful
  if (accuracy != null && accuracy > GPS_MIN_ACCURACY_M) {
    showToast(`📍 Weak GPS signal (±${Math.round(accuracy)}m) — refining…`);
    return;
  }
  ...
  // 2. Rolling average of recent accepted fixes, weighted toward accuracy
  _gpsRecent.push({ lng, lat, accuracy: accuracy ?? 15 });
  if (_gpsRecent.length > GPS_SMOOTH_WINDOW) _gpsRecent.shift();

  let wSum = 0, xSum = 0, ySum = 0;
  for (const f of _gpsRecent) {
    const w = 1 / Math.max(f.accuracy, 3); // more accurate fixes count more
    wSum += w; xSum += f.lng * w; ySum += f.lat * w;
  }
  _gpsSmoothed = [xSum / wSum, ySum / wSum];

  // 3. Snap onto the path network if we're close to one
  ...
  animateGPSMarkerTo(displayPoint, accuracy ?? 15);
  ...
  // 5. Live reroute if navigating from "My Location" and we've drifted off route
  if (_gpsActiveRouteTo && Router.isReady()) {
...
function removeGPSMarker() {
  ...
  _gpsRecent = []; _gpsSmoothed = null; _gpsDisplayed = null; _gpsHeading = null;
}
